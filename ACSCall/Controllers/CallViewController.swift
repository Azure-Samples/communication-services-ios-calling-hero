...
class CallViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    @IBAction func onShare(_ sender: UIButton) {
        ...
        let shareItems = [JoinIdShareItem(joinId: (callingContext.joinId?.lowercased())!, shareTitle: shareTitle)]
    ...
    private func initParticipantViews() {
        ...
        for (index, participant) in callingContext.displayedRemoteParticipants.enumerated() {
            ...
            let user = participant.identity as! CommunicationUserIdentifier
            let indexPath = IndexPath(item: index, section: 0)

            participantIdIndexPathMap[user.identifier] = indexPath
    ...
    private func updateParticipantViews(completionHandler: @escaping () -> Void) {
        ...
        for (index, participant) in callingContext.displayedRemoteParticipants.enumerated() {
            ...
            if let prevIndexPath = prevParticipantIdIndexPathMap[user.identifier],
    ...
    private func promptForTeamsAdmissionDenied() {
        let alert = UIAlertController(title: "Sorry, you were denied access to the meeting", message: nil, preferredStyle: .alert)
    ...
    private func promptForTeamsBeenRemoved() {
        let alert = UIAlertController(title: "You've been removed from this meeting", message: nil, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Dismiss", style: .default) { [weak self] _ in
